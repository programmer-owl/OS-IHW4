...
int pid = 0;
unsigned int bearAddrLen, hiveAddrLen, monAddrLen,
...
void notifyMonitor(char* mes) {
  if (sendto(sock, mes, RCVBUFSIZE, 0, (struct sockaddr *)&monAddr, sizeof(monAddr)) < 0)
...
int main(int argc, char *argv[]) {
  ...
  monAddrLen = sizeof(monAddr);
  // Устанавливаем адрес монитора
  if ((recvMsgSize = recvfrom(sock, buffer, RCVBUFSIZE, 0,
                            (struct sockaddr *)&monAddr, &monAddrLen)) < 0)
    DieWithError("recvfrom() failed");
  // сообщаем монитору, что получили его адрес
  if (sendto(sock, "1", 1, 0, (struct sockaddr *)&monAddr, sizeof(monAddr)) !=
      1)
    DieWithError("sendto() sent a different number of bytes than expected");
